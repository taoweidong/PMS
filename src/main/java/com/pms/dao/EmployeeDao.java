...
public class EmployeeDao {
	...
	public static ResultSet EmployeetList(Connection con, PageBean pageBean, Employee emp,
		...
		if (emp != null && StringUtils.isNotBlank(emp.getEmp_name())) {
			sb.append(" AND emp.emp_name LIKE '%" + emp.getEmp_name() + "%'");
		}
		if (emp != null && StringUtils.isNotBlank(emp.getEmp_no())) {
			sb.append("AND emp.emp_no LIKE '%" + emp.getEmp_no() + "%'");
		}
		if (emp != null && StringUtils.isNotBlank(emp.getEmp_sex())) {
			sb.append("AND emp.emp_sex LIKE '%" + emp.getEmp_sex() + "%'");
		}
		if (emp != null && StringUtils.isNotBlank(emp.getEmp_phone())) {
			sb.append("AND emp.EMP_Phone LIKE '%" + emp.getEmp_phone() + "%'");
		}
		if (emp != null && StringUtils.isNotBlank(emp.getEmp_address())) {
			sb.append("AND emp.EMP_Address LIKE '%" + emp.getEmp_address() + "%'");
		}
		// and zzmm.PS_TYPE = '102'
		if (emp != null && StringUtils.isNotBlank(emp.getPs_id())) {
			sb.append("AND zzmm.PS_TYPE LIKE '%" + emp.getPs_id() + "%'");
		}
		if (emp != null && StringUtils.isNotBlank(bbirthday)) {
			sb.append(" and TO_DAYS(emp.EMP_Birthday) >= TO_DAYS('" + bbirthday + "')");
		}
		if (emp != null && StringUtils.isNotBlank(ebirthday)) {
	...
	public static int EmployeeCount(Connection con, Employee student, String bbirthday,
		...
		if (StringUtils.isNotBlank(student.getEmp_name())) {
			sb.append(" AND emp.emp_name LIKE '%" + student.getEmp_name() + "%'");
		}
		if (StringUtils.isNotBlank(student.getEmp_no())) {
			sb.append("AND emp.emp_no LIKE '%" + student.getEmp_no() + "%'");
		}
		if (StringUtils.isNotBlank(student.getEmp_sex())) {
			sb.append("AND emp.emp_sex LIKE '%" + student.getEmp_sex() + "%'");
		}
		if (StringUtils.isNotBlank(student.getEmp_phone())) {
			sb.append("AND emp.EMP_Phone LIKE '%" + student.getEmp_phone() + "%'");
		}
		if (StringUtils.isNotBlank(student.getEmp_address())) {
			sb.append("AND emp.EMP_Address LIKE '%" + student.getEmp_address() + "%'");
		}
		if (StringUtils.isNotBlank(bbirthday)) {
			sb.append(" and TO_DAYS(emp.EMP_Birthday) >= TO_DAYS('" + bbirthday + "')");
		}
	 ...
	 * @throws Exception
	 */
	public static int EmlopyeeDelete(Connection con, String delIds) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static int EmployeeAdd(Connection con, Employee emp) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static int EmployeeRegister(Connection con, Employee emp) throws Exception {
		String sql = "INSERT INTO pms.t_employee() VALUES(?,?,?,?,?,?,?,?,?,?,?)";
		PreparedStatement pstmt = con.prepareStatement(sql);
	...
	public static int EmployeeModify(Connection con, Employee emp) {
		...
		try {
			if (emp != null && StringUtils.isNotBlank(emp.getEmp_pwd())) {
	 ...
	 * @throws SQLException
	 */
	public static boolean IsExistence(Connection con, String empNo) throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public static boolean IsInduction(Connection con, String emp_no) throws SQLException {
		// 检查：入职申请已审批通过，不允许删除
	...
	public static boolean IsLeader(Connection con, String emp_no) throws SQLException {
		// 检查：该用户是不是部门领导
