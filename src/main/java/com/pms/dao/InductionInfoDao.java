 ...
 * @author Taowd
 * @version 2018年8月31日
 * @see InductionInfoDao
 */
public class InductionInfoDao {

	/**
	 * 查询所有员工的入职信息--管理员权限才可以--即入职审批时的界面.
	 * @param con
	 * @param pageBean
	 * @param demp
	 * @param startDate
	...
	public static ResultSet InductionInfoList(Connection con, PageBean pageBean,
			InductionInfoBean demp, String startDate, String endDate) throws Exception {
		...
		if (demp != null && StringUtils.isNotBlank(demp.getInd_Id())) {
			sb.append(" and ii.IND_ID like '%" + demp.getInd_Id() + "%'");
		}
		// 员工号
		if (demp != null && StringUtils.isNotBlank(demp.getEmp_No())) {
			sb.append(" and emp.EMP_NO like '%" + demp.getEmp_No() + "%'");
		}
		// 审批状态
		if (demp != null && StringUtils.isNotBlank(demp.getExt1())) {
			sb.append(" and ii.EXT1 like '%" + demp.getExt1() + "%'");
		}
		// 在职状态
		if (demp != null && StringUtils.isNotBlank(demp.getInd_State())) {
			sb.append(" and ii.IND_STATE like '%" + demp.getInd_State() + "%'");
		}
		// 岗位编号
		if (demp != null && StringUtils.isNotBlank(demp.getPos_Name())) {
	...
	/**
	 * Author:Taowd 功能：个人申请职位调用的方法 开发日期：2017-4-13-下午9:05:21
	 * @param con
	 * @param induction
	 * @return
	 * @throws Exception
	 */
	public static int ApplyInduction(InductionInfoBean induction) throws Exception {
		// 职位申请:
	...
	/**
	 * Author:Taowd 功能：获取信息总数 开发日期：2017-4-13-上午8:18:42
	 * @param con
	 * @param inductionBean
	 * @return
	 * @throws SQLException
	...
	public static int InductionInfoCount(Connection con, InductionInfoBean inductionBean)
		...
		if (inductionBean != null && StringUtils.isNotBlank(inductionBean.getInd_Id())) {
			sb.append(" and ii.IND_ID like '%" + inductionBean.getInd_Id() + "%'");
		}
		if (inductionBean != null && StringUtils.isNotBlank(inductionBean.getEmp_No())) {
			sb.append(" and emp.EMP_NO like '%" + inductionBean.getEmp_No() + "%'");
		}
		if (inductionBean != null && StringUtils.isNotBlank(inductionBean.getPos_Id())) {
	...
	/**
	 * 检查该部门是否已经申请过了，如果申请过了，就不允许再次申请 开发日期.
	 * @param inductionBean
	 * @return
	 * @throws SQLException
	 */
	public static boolean IsExistence(InductionInfoBean inductionBean) throws SQLException {
		// 创建QueryRunner，需要提供数据库连接池对象
		QueryRunner qr = new QueryRunner(DbUtils.getDataSource());
		// 给出sql模板
	...
	/**
	 * Author:Taowd 功能：修改已经申请，但是没有提交的信息 开发日期：2017-4-14-下午1:23:49
	 * @param con
	 * @param induction
	 * @return
	 * @throws SQLException
	 */
	public static int ApplyInductionModify(InductionInfoBean induction) throws SQLException {
		// 创建QueryRunner，需要提供数据库连接池对象
	...
	/**
	 * Author:Taowd 功能：提交申请的逻辑 开发日期：2017-4-14-下午8:25:31
	 * @param con
	 * @param induction
	 * @return
	 * @throws SQLException
	 */
	public static int ApplyInductionApprove(InductionInfoBean induction) throws SQLException {
	...
	/**
	 * Author:Taowd 功能：检查申请状态是否可修改：即ext1=="33"?? 开发日期：2017-4-14-下午1:58:58
	 * @param con
	 * @param induction
	 * @return
	 * @throws SQLException
	...
	/**
	 * Author:Taowd 功能：检查当前状态 开发日期：2017-4-14-下午2:29:09
	 * @param con
	 * @param string
	 * @return
	 * @throws SQLException
	...
	/**
	 * Author:Taowd 功能：删除申请 开发日期：2017-4-14-下午2:31:26
	 * @param con
	 * @param delIds
	 * @return
	 * @throws SQLException
	...
	/**
	 * Author:Taowd 功能：检查是否为可审批状态--即ext1=="00" 开发日期：2017-4-17-下午1:05:04
	 * @param con
	 * @param string
	 * @return
	 * @throws SQLException
	...
	/**
	 * Author:Taowd 功能：管理员查询申请信息--特殊处理，排除未提交的申请 开发日期：2017-4-17-下午1:36:37
	 * @param con
	 * @param pageBean
	 * @param inductionBean
	 * @param startDate
	...
	public static ResultSet AdminInductionInfoList(Connection con, PageBean pageBean,
			InductionInfoBean demp, String startDate, String endDate) throws SQLException {
